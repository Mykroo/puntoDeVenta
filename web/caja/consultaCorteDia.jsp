...
<%
    ...
    String query = "select round(sum(total),2) as total, round(sum(descuento),2) as descuento, round(sum(pagoEfectivo),2) as efectivo, round(sum(pagoTarjeta),2) as tarjeta from ventatotal where date(fecha) = date(now());";
    float totalVentas = 0;
    float efectivo = 0;
    float tarjeta = 0;
    float descuento = 0; 
    ...
    if(n>0){
        ...
        objConn.Consult("select sum(((v.precioU-(select a.precioU from compraproducto a where a.idProducto = v.idProducto order by idcompraproducto desc limit 1))*v.cantidad)) as cantidad from ventaproducto v natural join ventatotal vt where date(vt.fecha) = date(now());");
        if(objConn.rs != null){
            try{
                ganancia = objConn.rs.getFloat("cantidad");                
            }catch(Exception e){}
        }
